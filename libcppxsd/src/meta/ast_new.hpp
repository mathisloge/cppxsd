...
{
    using Content = boost::variant<restriction, list, xsd_union>;
    Content content;
};
...
struct schema
{
    using Content = boost::variant<redefine, simpleType, complexType, group, attributeGroup, element, attribute>;
    std::vector<Content> content;
};
} // namespace cppxsd::meta
