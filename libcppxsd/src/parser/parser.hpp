...
class Parser final
{
  ...
  private:
    ...
    meta::element parseElement(const pugi::xml_node &node) const;
    meta::complexType parseComplexType(const pugi::xml_node &node) const;
    ...
    void parseCommonContent(meta::common_content &content, const pugi::xml_node &node) const;
    meta::simpleContent parseSimpleContent(const pugi::xml_node &node) const;
    meta::complexContent parseComplexContent(const pugi::xml_node &node) const;
    meta::extension parseExtension(const pugi::xml_node &node) const;
    void parseCommonSequence(meta::common_sequence &seq, const pugi::xml_node &node) const;
    meta::choice parseChoice(const pugi::xml_node &node) const;
    meta::sequence parseSequence(const pugi::xml_node &node) const;
    meta::all parseAll(const pugi::xml_node &node) const;
    meta::group parseGroup(const pugi::xml_node &node) const;
    meta::any parseAny(const pugi::xml_node &node) const;

    meta::GeneralAttributes getGeneralAttributes(const pugi::xml_node &node) const;
